...
def train(a):
    ...
    r"/home/m.lavaud/Documents/Zeus/I2/T_200_const_100_to_150_50_per_exp/batch_T_Const_1",)#[:9000]

    bi_mamba_stacks, conv_stack, dropout, learning_rate = a
    ...
    training_dataset = Dataset_all_data(
        all_data_set[:80], transform=False, pad=(max_particles, max_traj_len)
    )
    test_dataset = Dataset_all_data(
        all_data_set[-10:], transform=False, pad=(max_particles, max_traj_len)
    )
    ...
    # Training loop
    runnning_test_classification = []
    runnning_test_regression = []
    ...
    for epoch in range(max_epochs):
        with tqdm(dataloader, unit="batch", disable=False) as tepoch:
            ...
            test_loss_classification, test_loss_regression = evaluate_model(model,dataloader_test,regression_criterion)
            runnning_test_classification.append(test_loss_classification)
            runnning_test_regression.append(test_loss_regression)
    ...
    result = {"bi_mamba_stacks":bi_mamba_stacks,
            ...
            #   "running_classification_total_loss":running_classification_total_loss,
              ...
              "running_test_regression":runnning_test_regression,
              "runnning_test_classification":test_loss_classification,
             }
    return result
...
def evaluate_model(model, dataloader, criterion, device = "cuda"):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            total_regression_loss += regression_loss.item() * inputs.size(0)
            total_samples += inputs.size(0)
        

        return (total_classification_loss / total_samples), (total_regression_loss / total_samples)
